function adc_crlb_demo()
    % Calculates the CRLB for ADC (second diagonal element of inverse of Fisher information matrix), as a function of b and sigma
    function val = cost(x,b,sigma)
        if size(unique(b),1) > 1
            df = @(x,b) [exp(-b*x(2)) -b.*x(1).*exp(-b*x(2))];
            dy = df(x,b);
            I = (dy'*dy)./sigma^2;
            invI = inv(I+eps);
            val = invI(2,2);
